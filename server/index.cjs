...
const { open, writeFile, readFile, readdir } = require('node:fs/promises')
...
// Inside pkg snapshot __dirname resolves to the virtual snapshot path (e.g. /snapshot/tpt-emergency/server)
// so joining with '..' gives the project root inside the snapshot where dist assets live.
// Outside pkg we fall back to the real filesystem relative to this script.
function getDistPath() {
  const snapshotDist = join(__dirname, '..', 'dist')
  if (existsSync(snapshotDist)) return snapshotDist
...
const start = async () => {
  try {
    ...
    // Auto open browser when run as packaged exe
    if (isPackaged) {
